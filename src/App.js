...
export default function Home() {
  ...
  const handleSave = async () => {
    if (texture) {
      ...
      canvas.width = texture.image.width; // Adjust as needed
      canvas.height = texture.image.height; // Adjust as needed
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            // Now call the handleSubmit to submit the user data
  ...
  const handleSubmit = async (imageId) => {
    const data = {
      ...
      phone,
      email,
      sockDesign: parseInt(imageId, 10), // Include the image ID in the submission
    };
